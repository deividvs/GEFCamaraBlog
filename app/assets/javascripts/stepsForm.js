...
;( function( window ) {
	...
	stepsForm.prototype._nextQuestion = function() {
		...
		if ( this.supportsHTML5Forms ) {
			...
			if ( !input.checkValidity() ) {
				...
				input.setCustomValidity( 'Por favor preencha um email válido!' );
	...
	stepsForm.prototype._validate = function() {
		...
		if(input.type ==  'text' || input.type == 'email'){
			...
			var checked = document.querySelectorAll( '[name="' + input.name + '"]:checked').length;

			if( checked === 0 ) {
				this._showError( 'UNSELECTED' );
				return false;
			}
		}
	...
	stepsForm.prototype._showError = function( err ) {
		...
		switch( err ) {
			case 'UNSELECTED' :
				message = 'Por favor selecione uma das opções antes de continuar';
				break;
			case 'EMPTYSTR' :
